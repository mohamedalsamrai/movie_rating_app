...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    */
    if (
      popularMoviesState is LoadPopularMoviesLoading ||
      moviesByGenreState is AsyncLoading
    ) { return const _LoadingLayout(); }
    
    if (
      popularMoviesState is LoadPopularMoviesSuccess &&
      moviesByGenreState is AsyncData
    ) { 
      return _SuccessLayout(
        context, 
        popularMoviesState.movies,
        moviesByGenreState.value ?? {}
      );
    }
  ...
  Widget build(BuildContext context) => Scaffold(
    body: SingleChildScrollView(
      ...
      child: Column(
        children: [
          MovieCarousel(
            movies: popularMoviesList.sublist(0, 3)
          ),
          SizedBox(
